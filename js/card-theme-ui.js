...
(() => {
  ...
  function evaluateUnlocks({ announceUnlocks = true } = {}) {
    ...
    decks.forEach(deck => {
      if (deck.starter || unlocked.has(deck.id) || typeof deck.unlock !== "function") return;
  ...
  function refreshCollection() {
    const ownedDecks = decks.filter(deck => unlocked.has(deck.id));
