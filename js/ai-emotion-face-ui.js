...
(() => {
  ...
  const CHARACTER_NAMES = Object.freeze({
    Leo: "獅王", Toto: "龜龜", Foxy: "狐狸", Wolf: "灰狼", Pao: "熊貓",
    Shark: "鯊魚", Ace: "獵鷹", Momo: "猴王", Nori: "貓咪", Bruno: "棕熊",
    Dodo: "小鹿", Viper: "毒蛇", Nova: "外星人", "Unit-9": "機器人", Merlin: "魔法師",
  });

  // Every character has multiple visual reactions for every mood.
  // The animal/avatar stays as the main portrait; this badge supplies the reaction.
  ...
  const ACTION_VISUALS = Object.freeze({
    ...
    call: { icons: ["🤝", "👀", "🎟️", "✅"], label: "選擇跟注", tone: "call" },
    check: { icons: ["✋", "👀", "🆓", "⏸️"], label: "選擇過牌", tone: "check" },
    fold: { icons: ["💨", "🙈", "🫥", "🏳️", "🚪"], label: "暫時退場", tone: "fold" },
    win: { icons: ["🏆", "👑", "🎉", "💰", "✨"], label: "贏下底池", tone: "win" },
  });

  const MOOD_PHRASES = Object.freeze({
    fresh: "精神滿滿", calm: "冷靜觀察", confident: "氣勢全開",
    tilted: "正在發怒", cautious: "提高警戒",
  });
  ...
  function pick(items, player, stateKey) {
    ...
    const seed = `${player.name}|${stateKey}|${state.handNumber}|${player.streak || 0}|${player.position}|${player.lastAction || ""}`;
  ...
  function visualMeta(player) {
    ...
    const actionKey = isWinner ? "win"
      : player.folded ? "fold"
      : isThinking ? "thinking"
      : ["allin", "raise", "call", "check"].includes(player.lastAction) ? player.lastAction
      : null;
  ...
  function installStyles() {
    ...
    style.textContent = `
      html body .seat-header { grid-template-rows: repeat(2, 23px) !important; min-height: 66px !important; }
      html body .seat-header .seat-identity { grid-row: 1 / 3 !important; }
      html body .seat-header .seat-status { grid-row: 2 !important; }
      html body .seat-header > .emotion-chip { display: none !important; }
      html body .player-emoji, html body .ai-profile-avatar { position: relative !important; overflow: visible !important; }
      .ai-emotion-face-badge {
        position: absolute; z-index: 4; right: -9px; bottom: -8px; display: grid; place-items: center;
        width: 21px; height: 21px; border: 1px solid rgba(255,255,255,.68); border-radius: 999px;
        background: rgba(5,13,16,.95); font-size: .78rem; line-height: 1;
        box-shadow: 0 4px 11px rgba(0,0,0,.34); pointer-events: none; transform-origin: center;
      }
      .ai-emotion-face-badge.is-profile { right: -8px; bottom: -7px; width: 25px; height: 25px; font-size: .94rem; }
      .ai-emotion-face-badge.tone-confident, .ai-emotion-face-badge.tone-win {
        border-color: rgba(255,215,105,.88); box-shadow: 0 4px 11px rgba(0,0,0,.34), 0 0 12px rgba(255,200,70,.34);
        animation: aiMoodGlow 1100ms ease-in-out infinite alternate;
      }
      .ai-emotion-face-badge.tone-tilted, .ai-emotion-face-badge.tone-allin, .ai-emotion-face-badge.tone-raise {
        border-color: rgba(255,113,76,.9); box-shadow: 0 4px 11px rgba(0,0,0,.34), 0 0 13px rgba(255,90,45,.38);
      }
      .ai-emotion-face-badge.tone-tilted { animation: aiMoodAngry 420ms ease-in-out infinite alternate; }
      .ai-emotion-face-badge.tone-thinking { animation: aiMoodThink 850ms ease-in-out infinite alternate; }
      .ai-emotion-face-badge.tone-cautious { border-color: rgba(140,205,255,.72); }
      .ai-emotion-face-badge.tone-fold { opacity: .72; filter: grayscale(.25); }
      .ai-profile-tags .emotion-chip, .ai-profile-now > span[data-emotion], .seat-header > .emotion-chip { display: none !important; }
      @keyframes aiMoodGlow { from { transform: scale(.96); } to { transform: scale(1.1); } }
      @keyframes aiMoodAngry { from { transform: rotate(-7deg) scale(1); } to { transform: rotate(7deg) scale(1.08); } }
      @keyframes aiMoodThink { from { transform: translateY(1px) scale(.96); } to { transform: translateY(-2px) scale(1.06); } }
      @media (prefers-reduced-motion: reduce) { .ai-emotion-face-badge { animation: none !important; } }
    `;
  ...
  window.AiEmotionFaceUI = { characterMoods: CHARACTER_MOODS, refresh: decorateEmotionFaces };
})();
