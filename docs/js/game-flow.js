...
function startHand() {
  ...
  const humanStack = previousPlayers[0]?.stack > 0 ? previousPlayers[0].stack : STARTING_STACK;
  ...
  if (humanWasBusted) {
    log(`🦉 Owl 籌碼歸零，重新買入 ${STARTING_STACK}。`);
  }
  ...
  replacements.forEach(({ replaced, player }) => {
    log(`${replaced.emoji} ${replaced.name} 籌碼歸零，淘汰離桌。${player.emoji} ${player.name} 帶入 ${STARTING_STACK} 加入牌桌。`);
  });
