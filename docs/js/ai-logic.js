...
function botAction(player) {
  ...
  const shortStackJam = stackInBigBlinds <= SHORT_STACK_JAM_BB
    ...
    && Math.random() < player.aggression * 0.62;
  const premiumJam = stackInBigBlinds <= 18
    && premiumHand
    && needed >= BIG_BLIND
    && Math.random() < player.aggression * 0.28;
